...
class CamerasControllerTest < ActionController::TestCase
  ...
  test "should create camera" do
    assert_difference('Camera.count') do
      post :create, camera: { address: @camera.address, description: @camera.description, name: @camera.name }
    end
  ...
  test "should update camera" do
    put :update, id: @camera, camera: { address: @camera.address, description: @camera.description, name: @camera.name }
    assert_redirected_to camera_path(assigns(:camera))
  end
